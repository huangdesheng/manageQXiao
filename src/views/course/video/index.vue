      ...
      <el-table :data="tableData" style="width: 100%" size="small" border>
        ...
        <el-table-column label="推荐首页" align="center">
          <template slot-scope="scope" v-if="scope.row.status === 1">
            <el-button size="mini" type="text" v-if="scope.row.recommend === 1" @click="handleRecommend(scope.row.id)" style="color:#FC7878">取消</el-button>
...
export default {
  ...
  methods: {
    // 查询历史标签====================================
    async courseTag() {
      let res =  await service.courseTag(1)
      if(res.errorCode === 0) {
        this.tag = res.data
      }else{
        this.tag = []
