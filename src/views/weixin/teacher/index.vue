  ...
  <div class="page"
        ...
        <el-form class="demo-form-inline"
          ...
          <el-form-item label="班级">
            <el-select v-model="query.classId"
                       placeholder="选择班级">
              <el-option v-for="(item,index) in classListCheck"
                         :key="index"
          ...
          <el-form-item label="老师姓名">
            <el-input v-model="query.teacherName"
                      placeholder="请输入老师姓名"
          ...
          <template slot-scope="scope">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(name,index) in scope.row.classList"
                                  :key="index">{{ name.className }}</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
    ...
    <el-dialog top="40px"
      ...
      <el-form :rules="rules"
        ...
        <template v-if="parseInt(radio) === 0">
          <el-row :gutter="5"
                  v-for="(items,index) in form.classList"
                  :key="index">
            <el-col :span="8">
              <el-form-item label="任教班级"
                ...
                <el-select collapse-tags
                           ...
                           value-key="classId"
                           placeholder="选择任教班级">
                  <el-option v-for="(item,index) in classList.filter(item=>item.classId != 0)"
                             :key="index"
                             :label="item.className"
                             :value="item.classId"></el-option>
                </el-select>
            ...
            <el-col :span="8">
              <el-form-item label="类别"
                            prop="classAdmin">
                <el-select v-model="items.classAdmin"
                           placeholder="选择职位类别">
            ...
            <el-col :span="8">
              <el-form-item>
                <el-button size="mini"
                           ...
                           circle
                           type="danger"
                           @click="handleDeleteteacher(index)"></el-button>
              </el-form-item>
            </el-col>
          </el-row>
          <el-form-item>
            <el-button icon="el-icon-plus"
                       size="mini"
                       type="primary"
                       @click="handleAddTeacher">新增任教班级</el-button>
          </el-form-item>
        </template>
      </el-form>
      <span slot="footer"
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let obj = Object.assign({}, this.form, {
            schoolId,
            admin: parseInt(this.radio),
            classList: parseInt(this.radio) === 1 ? null : this.form.classList,
          });
          if (this.isShow) {
            this.addTeacher(obj);
          } else {
            this.updateTeacher(obj);
          }
        }
      });
    },
